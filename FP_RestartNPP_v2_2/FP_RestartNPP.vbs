...
Sub MAIN()
	...
	Dim o_WshShell, o_FileSysObj, o_WMIServiceObj, o_FileObj, o_Append_StreamFileObj
	...
	On Error Resume Next
		Set o_Append_StreamFileObj = o_FileSysObj.OpenTextFile(s_session_filefull, i_for_appending, False)
		i_errnum = Err.Number
	On Error GoTo 0
	If i_errnum <> 0 Then
		MsgBox "Unable to access the temporary session file" & vbCr & s_session_filefull & vbCr & vbCr & s_restartfailed, _
			vbExclamation, S_CURVBS
		Exit Sub
	End If
