# PythonScript that restarts Notepad++ and restore the session (all opened files, text selection, cursor position)
...
# (i) visual glitch : a small flicker is caused by the fact that Notepad++ is locked during the restart dialog
...
class C_Restart_NPP():
	...
	def __init__(self, i_s_wait_npp_close, i_s_wait_npp_start):
		...
		from FP__Lib_Window import C_Block_UnBlock_Input
	...
	def Restart(self):
		...
		if b_restarted:
			...
			if not(self.rest_session_filefull is None):
				...
				try:
					self.os.remove(self.rest_session_filefull)	# and clean-up the temporary session file (the VBS could not do it)
				except:
					pass
